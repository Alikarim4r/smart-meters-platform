...
class _ChartTypeRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isComparing = comparisonSelection.isNotEmpty;
    final periodKey = utilityChartPeriodKey(
      siteId: siteId,
      categoryCode: system.categoryCode,
    );
    ...
    final chartType = ref.watch(
      utilityChartTypeProvider(utilityChartTypeKey(siteId, system.categoryCode)),
    );
    final availableChartTypes = isComparing
        ? comparisonChartTypesForUtility(system, bucket: resolved.range.bucket)
...
class _AnalyticsChart extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final availableChartTypes = isComparing
        ? comparisonChartTypesForUtility(system, bucket: rangeOverride.bucket)
        : chartTypesForUtility(
            system,
...
class _UtilityChartBody extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (chartType == UtilityChartType.cop) {
      return _CopChartSection(
        siteId: siteId,
        periodState: periodState,
    ...
    return bundleAsync.when(
      ...
      data: (bundle) => switch (chartType) {
        ...
        UtilityChartType.cop => const SizedBox.shrink(),
      },
    );
  }
}
...
class _CopChartSection extends ConsumerWidget {
  const _CopChartSection({
    required this.siteId,
    required this.periodState,
    required this.businessDate,
  });
      ...
      data: (groups) {
        if (groups.isEmpty) {
          return ChartEmptyPlaceholder(
            message: s.isAr
                ? 'لا توجد مجموعات COP لهذا الموقع.'
                : 'No COP groups configured for this site.',
          );
        }
        ...
        return trendAsync.when(
          ...
          data: (result) {
            if (!result.hasData) {
              return ChartEmptyPlaceholder(message: s.noReadingsForPeriod);
            }
            final points = [
            ...
            return SingleSeriesLineChart(
              points: points,
              color: DashboardPalette.btu,
              unitLabel: 'COP',
              emptyMessage: s.noReadingsForPeriod,
