-- 046: Allow site managers (can_manage_site) to force-delete meters, and
-- clear utility-network assets that block meter deletion (ref_meter_id RESTRICT).
...
begin
  ...
  if cardinality(v_asset_ids) > 0 then
    -- Connections involving nodes of these assets
    ...
    -- Ports cascade with assets when asset row is removed.
    delete from public.site_utility_assets
    where id = any (v_asset_ids);
  end if;

  -- Legacy 031 graph (if present) — column is ref_meter_id
...
comment on function public.admin_force_delete_meter(uuid) is
  'Force-delete meter for super_admin or site managers (can_manage_site), including utility-network asset cleanup.';
